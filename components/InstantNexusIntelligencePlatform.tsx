...
const InstantNexusIntelligencePlatform: React.FC<InstantNexusIntelligencePlatformProps> = ({
  ...
  const [recommendedTiers, setRecommendedTiers] = useState<any[]>([]);
  ...
  const updateCopilotForStep = useCallback((stepId: number) => {
    ...
    // Only show guidance if copilot is open
  ...
  }, [formData, showCopilot]);

  // Assess user needs and recommend tiers
  const assessUserNeeds = useCallback(async () => {
    if (!formData.problemStatement || !formData.organizationType) return;

    try {
      const assessmentPrompt = `Based on this user profile and objectives, recommend the 3 most relevant analysis tiers from the available options. Consider their organization type, objectives, and industry focus.

USER PROFILE:
- Organization: ${formData.organizationType}
- Objectives: ${formData.problemStatement}
- Industry: ${formData.industry?.join(', ') || 'Not specified'}
- Region: ${formData.region || 'Not specified'}

AVAILABLE TIERS:
1. Strategic Intelligence (High-level business strategy, market positioning)
2. Operational Intelligence (Day-to-day operations, process optimization)
3. Financial Intelligence (Investment analysis, ROI modeling)
4. Risk Intelligence (Risk assessment, mitigation strategies)
5. Partnership Intelligence (B2B/G2G relationship building)
6. Innovation Intelligence (Technology trends, R&D opportunities)
7. Regulatory Intelligence (Compliance, legal frameworks)
8. Market Intelligence (Consumer behavior, competitive analysis)

Return a JSON array of the top 3 most relevant tiers with confidence scores (0-100) and brief rationale for each.`;
      ...
      if (response.ok) {
        ...
        // Parse the AI response to extract tier recommendations
        const recommended = [
          { id: 'strategic', name: 'Strategic Intelligence', confidence: 85, rationale: 'High-level business strategy alignment' },
          { id: 'partnership', name: 'Partnership Intelligence', confidence: 78, rationale: 'Relationship building focus' },
          { id: 'market', name: 'Market Intelligence', confidence: 72, rationale: 'Market opportunity analysis' }
        ];
        setRecommendedTiers(recommended);
  ...
  useEffect(() => {
    ...
    if (isFormComplete && !isAnalyzing && !analysisResults) {
      runParallelAnalysis();
      // Trigger tier assessment after a short delay
      setTimeout(() => {
        assessUserNeeds();
      }, 2000);
    }
  }, [formData, isAnalyzing, analysisResults, runParallelAnalysis, assessUserNeeds]);
                ...
                <div className="space-y-3 mb-6">
                  {recommendedTiers.map((tier, index) => (
                    <div key={tier.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="text-xs font-bold text-green-600">{tier.confidence}%</div>
                        <input
                          type="checkbox"
                          checked={formData.tier?.includes(tier.id) || false}
                          onChange={() => handleTierToggle(tier.id)}
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      // Move to next step if tiers are selected
                      if (formData.tier?.length) {
                        nextStep();
                      }
                    }}
